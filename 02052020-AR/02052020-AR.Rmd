...
```{r density_plots, echo=FALSE, fig.height=4.2}
exp_1 %>% filter(cell != 'C') %>%
...
Here CEBPa and CEBPb are plotted together with the linear model of the 1:1 stoichiometry between the two proteins from the calibration construct. PPARg abundance is represented by the color gradient, with darker blue representing high PPARg. Interestingly, it seems that the higher dox concentrations have a sub-population of high PPARg expressing cells that are below the diagonal (higher CEBPa to CEBPb) and higher in both CEBPa and CEBPb. It would be very interesting to see how this population moves during the time course of differentiation or under a dox titration with a later time point.
...
p1 <- exp_1 %>% filter(cell != 'C',
  ...
  ggplot(., aes(x = Cebpb,
                y = Pparg)) +
  geom_point(size = 0.2, alpha = 0.2) + facet_grid(dox~interval) +
...
plot_grid(p1,p2, ncol = 1, labels = "AUTO", rel_heights = c(7,1.2))
```
